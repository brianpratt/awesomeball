...
FILES_TO_IGNORE = [".svn", "." ,"..", ".DS_Store", SOURCE_XIB_FOLDER, "Localizable.strings", ""]

# Iterate through the current directory.
Dir.entries(STRINGS_PATH).each do |localized_folder|
  if (!FILES_TO_IGNORE.include?(localized_folder) and localized_folder.slice(localized_folder.length-6,localized_folder.length-1) == ".lproj")
    ...
    Dir.entries(STRINGS_PATH + "/" + localized_folder).each do |strings_file|
      if (!FILES_TO_IGNORE.include?(strings_file) and strings_file.slice(strings_file.length-8,strings_file.length-1) == ".strings")
        ...
        command = "ibtool --strings-file \"#{strings_path}\" --write \"#{localized_folder}/#{filename}.xib\" \"#{source_xib}.xib\""
        results = %x[#{command}]
        if results.length > 0
          puts "FAILURE: #{command}:\n#{results}"
        else
          puts "SUCCESS: #{localized_folder}/#{filename}.xib"
        end
      end
    end
  end
